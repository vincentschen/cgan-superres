...
def unzip(save_path, dirpath, extracted_dirpath):
  ...
  os.remove(save_path)
  os.rename(os.path.join(dirpath, zip_dir), os.path.join(dirpath, extracted_dirpath))

def handle_download(target_filename, dirpath): 
  drive_id = "0B7EVK8r0v71pZjFTYXZWM3FlRnM"
  save_path = os.path.join(dirpath, target_filename)
...
def download_celeb_a(dirpath):
  data_dir = 'celebA'
  data_dirpath = os.path.join(dirpath, data_dir)
  ...
  dataset_path = handle_download("img_align_celeba.zip", data_dirpath)
  unzip(dataset_path, data_dirpath, 'images')

#  _ = handle_download("list_eval_partition.txt", data_dirpath)
#  _ = handle_download("list_attr_celeba.txt", data_dirpath)
