...
def create_model(sess, features, labels):
    ...
    with tf.variable_scope('gene') as scope:
        ...
        print("gene output", gene_output.shape)

        scope.reuse_variables()

        gene_moutput, _ = _generator_model(sess, gene_minput, labels, channels)
        print("gene moutput", gene_moutput.shape)
...
def create_generator_loss(sess, disc_output, gene_output, features, labels):
    ...
    # print("gen_features", sess.run(features))
    # print("gen_output", sess.run(gene_output))

    # send labels through class, get new labels
    # send gene_output and new labels through classifier, get cross entropy
    # add cross entropy to gene loss
...
def create_optimizers(gene_loss, gene_var_list,
    ...
    class_loss = tf.placeholder(dtype=tf.float32, shape=())

    gene_minimize = gene_opti.minimize(gene_loss+class_loss, var_list=gene_var_list, name='gene_loss_minimize', global_step=global_step)

    disc_minimize     = disc_opti.minimize(disc_loss, var_list=disc_var_list, name='disc_loss_minimize', global_step=global_step)
    
    return (global_step, learning_rate, gene_minimize, disc_minimize, class_loss)
