...
tf.app.flags.DEFINE_integer('test_vectors', 5000,
...
def _train():
    ...
    gene_loss = srez_model.create_generator_loss(sess, disc_fake_output, gene_output, train_features, train_labels)
    disc_real_loss, disc_fake_loss = \
                     srez_model.create_discriminator_loss(disc_real_output, disc_fake_output)
    disc_loss = tf.add(disc_real_loss, disc_fake_loss, name='disc_loss')
    
    (global_step, learning_rate, gene_minimize, disc_minimize, class_loss) = \
            srez_model.create_optimizers(gene_loss, gene_var_list,
                                         disc_loss, disc_var_list)

    # Train model
    train_data = TrainData(locals())
    # print("TD.", sess.run(train_data.gene_output))
